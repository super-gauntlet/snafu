...
[_unit,_debug] spawn {
	...
	if (([_unit,INC_regEnySide,20] call INCON_ucr_fnc_groupsWithPID) || {([_unit,INC_asymEnySide,20] call INCON_ucr_fnc_groupsWithPID)}) then {
		...
		if (!(_unit getVariable ["INC_compWarningPlayed", false]) && (_unit getVariable ["has_gone_undercover", false])) then {
			[ 
				[ 
					["ATTN:", "<t align = 'center' shadow = '1' size = '0.7' font='PuristaBold'>%1</t>"], 
					["Your Description Has Been Shared To The Enemy", "<t align = 'center' shadow = '1' size = '0.7'>%1</t><br/>"], 
					["Break contact and change your disguise to go back undercover...", "<t align = 'center' shadow = '1' size = '1.0'>%1</t>", 15] 
				], safeZoneX, (safeZoneY + safeZoneH * 0.8)
			] remoteExec ["BIS_fnc_typeText", _unit];
			_unit setVariable ["INC_compWarningPlayed", true, true];
			[_unit] spawn {
				sleep 60;
				_unit = _this select 0;
				_unit setVariable ["has_gone_undercover", false, true];
			};
		};
		...
		_unit setVariable ["INC_compWarningPlayed", false, true];
	...
	//Otherwise he is no longer compromised
		...
		// Publicize isCompromised to false.
		_unit setVariable ["INC_isCompromised", false];
		_unit setVariable ["INC_compWarningPlayed", false];
