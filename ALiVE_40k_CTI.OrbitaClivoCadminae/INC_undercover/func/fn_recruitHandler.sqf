...
switch (_operation) do {
    ...
    case "recruitCiv": {
        ...
        _unit addAction [
        	"<t color='#33FFEC'>Recruit</t>", {
        		...
        		if !((weaponLowered _undercoverUnit) || (currentWeapon _undercoverUnit == "") || (currentWeapon _undercoverUnit == "Throw")) exitWith {
        		...
        		[[_civ,_undercoverUnit],"recruitAttempt"] remoteExecCall ["INCON_ucr_fnc_recruitHandler", 2];
    ...
    case "recruitAttempt": {
        private ["_undercoverGroup","_percentage","_civComment", "_cluster", "_hostility", "_hostility_array"];

        _input params ["_civ","_undercoverUnit"];

        _cluster = [position _civ] call SG_fnc_get_nearest_cluster;
        _hostility_array = [_cluster, "hostility"] call ALiVE_fnc_hashGet;
        _hostility = [_hostility_array, str side _undercoverUnit] call ALiVE_fnc_hashGet;

        _undercoverGroup = count units group _undercoverUnit;

        _percentage = (linearConversion [100, -100, (_hostility), 5, 70, true]);
        ...
        if (_percentage > 30) then {
            ...
            if ((_percentage > (random 100)) && {_undercoverUnit getVariable ["isUndercover", false]} && {!(_undercoverUnit getVariable ["INC_isCompromised", false])}) then {
                ...
                if (_undercoverGroup < (2 + (random 5))) then {
                    ...
                    _civComment = selectRandom ["This is for Cyril.","I'll get my fighting hat on.","I'm bored, why not.","I'll join you, but only because they ran over my rabbit.","I should really know better.","I'm going to regret this.","Fuck it, let's go.","My wife is going to kill me for this.","Well it's better than farming."];
                    [[_civ, _civComment] remoteExec ["globalChat",0]];
                    (group _civ) setGroupOwner (owner _undercoverUnit);
                    [[_civ,_undercoverUnit],"recruitSuccess"] remoteExecCall ["INCON_ucr_fnc_recruitHandler",_undercoverUnit];
                ...
                _civComment = selectRandom ["You stink.","Have a shower.","I'm good thanks.","My brother likes to do stupid things. Ask him.","Try my cousin, he's a lunatic.","My piles are more threatening than you.","They will kill us all.","There is nothing we can do.","What have you ever done for us?","Who are you?","Why should I join you?","Get out of here foreigner.","I would but I've got this really important thing on.","Prove yourself and I'll join you next time.","Yeah... good luck.","What's the point?","I'll fight them on my own time."];
                [[_civ, _civComment] remoteExec ["globalChat",0]];
