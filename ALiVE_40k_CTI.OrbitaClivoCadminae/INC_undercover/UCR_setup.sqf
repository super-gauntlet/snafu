...
_undercoverUnitSide = east;             //What side is/are the undercover unit(s) on? (Can be east, west or independent - only one side supported)
...
_civilianVests = ["V_ALiVE_Suicide_Vest", "V_Press_F", "vn_o_vest_04", "vn_o_vest_05"];
...
_civilianBackpacks = ["B_TacticalPack_blk"];
...
_incogFactions = ["BLU_F"]; //Array of enemy factions whose items and vehicles will allow the player to impersonate the enemy
...
_trespassMarkers = ["MIL_BASE00","MIL_BASE01","MIL_BASE02"];
...
_civWpnArray = ["hgun_P07_F"];

//Items that civilians may carry
_civItemArray = ["FirstAidKit"];
